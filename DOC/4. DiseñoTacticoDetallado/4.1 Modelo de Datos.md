# 4.1 Modelo de Datos

Modelo entidad-relación (MER) de Llavero sobre PostgreSQL — las 20 entidades del modelo de dominio (`2.1 Modelo Dominio Anémico.md`) más `SesionRefresh` (mecanismo propio de rotación/revocación de JWT del módulo `auth`, sin equivalente en el modelo de dominio porque es infraestructura de autenticación, no un concepto de negocio), ahora con claves primarias/foráneas explícitas y cardinalidad real de base de datos. Dividido en 5 diagramas por área funcional para que sean legibles; las entidades compartidas (`Usuario`, `Comunidad`, `Ubicacion`, `Salon`) se declaran completas una sola vez y se referencian en los demás diagramas solo para mostrar la relación.

DDL completo: [`4.5 DDL.sql`](./4.5%20DDL.sql). Las claves primarias son `UUID`, pero el valor lo genera el backend (Django: `UUIDField(default=uuid.uuid4)`) antes del INSERT, no la base de datos.

## Identidad, catálogos y espacios

```mermaid
erDiagram
    ROL ||--o{ USUARIO : tiene
    UBICACION ||--o{ USUARIO : "tiene asignada"
    TIPO_PERSONA ||--o{ COMUNIDAD : "es de tipo"
    BLOQUE ||--o{ SALON : pertenece
    TIPO_SILLETERIA ||--o{ SALON : tiene
    USUARIO ||--o{ SESION_REFRESH : "tiene sesiones de"
    USUARIO ||--o{ CODIGO_LOGIN_TEMPORAL : "genera al loguearse"

    ROL {
        uuid id PK
        string nombre
    }
    USUARIO {
        uuid id PK
        string nombre
        string email_institucional
        string oid_microsoft
        uuid rol_id FK
        uuid ubicacion_id FK
        boolean activo
    }
    SESION_REFRESH {
        uuid id PK
        uuid usuario_id FK
        string jti
        datetime fecha_emision
        datetime fecha_expiracion
        datetime fecha_revocacion
    }
    CODIGO_LOGIN_TEMPORAL {
        uuid id PK
        string codigo
        uuid usuario_id FK
        datetime fecha_emision
        datetime fecha_expiracion
        boolean usado
    }
    UBICACION {
        uuid id PK
        string nombre
        boolean permite_prestamo_llaves
        boolean permite_devolucion_llaves
        boolean permite_prestamo_equipos
    }
    TIPO_PERSONA {
        uuid id PK
        string nombre
    }
    COMUNIDAD {
        uuid id PK
        string numero_documento
        string nombre
        uuid tipo_persona_id FK
        string id_carnet
        string correo
        string numero_contacto
        string facultad
    }
    BLOQUE {
        uuid id PK
        string nombre
    }
    TIPO_SILLETERIA {
        uuid id PK
        string nombre
    }
    SALON {
        uuid id PK
        string nombre
        uuid bloque_id FK
        uuid tipo_silleteria_id FK
        int cantidad_sillas
        int cantidad_mesas
    }
```

## Disponibilidad de salón

```mermaid
erDiagram
    SALON ||--o{ PROGRAMACION : "se dicta en"
    COMUNIDAD ||--o{ PROGRAMACION : dicta
    SEMESTRE ||--o{ PROGRAMACION : pertenece

    SALON ||--o{ RESERVA_SEMESTRAL : ocupa
    COMUNIDAD ||--o{ RESERVA_SEMESTRAL : solicita
    SEMESTRE ||--o{ RESERVA_SEMESTRAL : pertenece

    SALON ||--o{ RESERVA_INDIVIDUAL : ocupa
    COMUNIDAD ||--o{ RESERVA_INDIVIDUAL : solicita

    SALON {
        uuid id PK
    }
    COMUNIDAD {
        uuid id PK
    }
    SEMESTRE {
        uuid id PK
        string codigo
        date fecha_inicio
        date fecha_fin
    }
    PROGRAMACION {
        uuid id PK
        uuid salon_id FK
        uuid docente_id FK
        uuid semestre_id FK
        string dia
        time hora_inicio
        time hora_fin
        string materia
    }
    RESERVA_SEMESTRAL {
        uuid id PK
        uuid salon_id FK
        uuid solicitante_id FK
        uuid semestre_id FK
        string dia
        time hora_inicio
        time hora_fin
        uuid grupo_id
        boolean creado_manualmente
    }
    RESERVA_INDIVIDUAL {
        uuid id PK
        uuid salon_id FK
        uuid solicitante_id FK
        date fecha
        time hora_inicio
        time hora_fin
        string motivo
        string estado
    }
```

## Llaves

```mermaid
erDiagram
    SALON ||--o{ LLAVE : corresponde
    COMUNIDAD ||--o{ LLAVE : "es titular de"
    COMUNIDAD ||--o{ LLAVE : reclama
    USUARIO ||--o{ LLAVE : entrega
    USUARIO ||--o{ LLAVE : recibe
    UBICACION ||--o{ LLAVE : "entrega en"
    UBICACION ||--o{ LLAVE : "devuelve en"
    LLAVE |o--o| NOVEDAD : "puede generar"
    PROGRAMACION |o--o| LLAVE : genera
    RESERVA_SEMESTRAL |o--o| LLAVE : genera
    RESERVA_INDIVIDUAL |o--o| LLAVE : genera

    SALON { uuid id PK }
    COMUNIDAD { uuid id PK }
    USUARIO { uuid id PK }
    UBICACION { uuid id PK }
    PROGRAMACION { uuid id PK }
    RESERVA_SEMESTRAL { uuid id PK }
    RESERVA_INDIVIDUAL { uuid id PK }

    LLAVE {
        uuid id PK
        uuid salon_id FK
        uuid docente_titular_id FK
        uuid reclamado_por_id FK
        string origen
        string tipo_entrega
        string tipo_devolucion
        uuid usuario_entrega_id FK
        uuid usuario_recibe_id FK
        uuid ubicacion_entrega_id FK
        uuid ubicacion_devolucion_id FK
        uuid novedad_id FK
        datetime fecha_hora_entrega
        datetime fecha_hora_devolucion
        string estado
    }
    NOVEDAD {
        uuid id PK
    }
```

## Equipos

```mermaid
erDiagram
    COMUNIDAD ||--o{ PRESTAMO : solicita
    USUARIO ||--o{ PRESTAMO : gestiona
    UBICACION ||--o{ PRESTAMO : "ocurre en"
    PRESTAMO ||--o{ DETALLE_PRESTAMO : contiene
    EQUIPO ||--o{ DETALLE_PRESTAMO : referencia
    DETALLE_PRESTAMO |o--o| NOVEDAD : "puede generar"
    PRESTAMO ||--o{ DEVOLUCION : pertenece
    USUARIO ||--o{ DEVOLUCION : recibe
    UBICACION ||--o{ DEVOLUCION : "ocurre en"

    COMUNIDAD { uuid id PK }
    USUARIO { uuid id PK }
    UBICACION { uuid id PK }
    NOVEDAD { uuid id PK }

    EQUIPO {
        uuid id PK
        string nombre
        string marca
        string codigo_inventario
        string codigo_barras
        string estado
    }
    PRESTAMO {
        uuid id PK
        uuid solicitante_id FK
        uuid usuario_prestamista_id FK
        uuid ubicacion_id FK
        string estado
        datetime fecha_creacion
    }
    DETALLE_PRESTAMO {
        uuid id PK
        uuid prestamo_id FK
        uuid equipo_id FK
        uuid novedad_id FK
        string estado_equipo
        datetime fecha_entrega
        datetime fecha_devolucion
    }
    DEVOLUCION {
        uuid id PK
        uuid prestamo_id FK
        uuid usuario_recibe_id FK
        uuid ubicacion_id FK
        datetime fecha
        boolean es_completa
    }
```

## Soporte

```mermaid
erDiagram
    COMUNIDAD ||--o{ MONITOR : delega
    COMUNIDAD ||--o{ MONITOR : "es delegado en"
    USUARIO ||--o{ NOVEDAD : registra
    COMUNIDAD ||--o{ NOTIFICACION : recibe
    USUARIO ||--o{ NOTIFICACION : envia
    UBICACION ||--|| CONFIGURACION : "es por defecto en"

    COMUNIDAD { uuid id PK }
    USUARIO { uuid id PK }
    UBICACION { uuid id PK }

    MONITOR {
        uuid id PK
        uuid docente_titular_id FK
        uuid monitor_delegado_id FK
        string materia
        string aula
        string dia
        string horario
        boolean activo
    }
    NOVEDAD {
        uuid id PK
        string categoria
        string descripcion
        string estado
        string solucion
        uuid registrado_por_id FK
    }
    NOTIFICACION {
        uuid id PK
        uuid destinatario_id FK
        string tipo
        string asunto
        string mensaje
        string estado_envio
        uuid enviado_por_id FK
    }
    CONFIGURACION {
        uuid id PK
        int limite_antes_mora_minutos
        int max_reintentos_recordatorio
        string plantilla_recordatorio
        uuid ubicacion_defecto_id FK
    }
```

## Explicación de cada entidad

**Rol** — Catálogo cerrado de 3 filas (`admin`, `auxiliar`, `portero`). Normalizado como tabla en vez de enum para no repetir la lista de valores válidos en cada capa del sistema.

**Usuario** — Cuenta de una persona del personal (Admin, Auxiliar o Portero). Se precrea con `rol_id` y `ubicacion_id` ya asignados; el login por Office 365 solo la vincula por `email_institucional`/`oid_microsoft`, no la crea. `activo=false` revoca el acceso sin borrar el histórico de lo que esa persona procesó.

**Ubicacion** — Los puntos físicos de operación (oficina principal, porterías). Los tres `permite_*` booleanos existen porque no todas las ubicaciones habilitan las mismas operaciones (ej. una portería puede no estar habilitada para préstamo de equipos).

**TipoPersona** — Catálogo cerrado (`docente`, `estudiante`, `empleado`) para clasificar a `Comunidad`.

**Comunidad** — Docentes, estudiantes y empleados, sincronizados por el ETL institucional (no se editan a mano en Llavero). `id_carnet` es lo que identifica a la persona cuando pasa su credencial por el lector.

**Bloque** / **TipoSilleteria** — Catálogos simples de apoyo a `Salon`.

**Salon** — Un espacio físico reservable. `cantidad_sillas`/`cantidad_mesas` reemplazan un único número de "capacidad" porque el mobiliario puede ser de sillas individuales, de mesas, o una combinación.

**Semestre** — Delimita la vigencia de `Programacion` y `ReservaSemestral` — ambas existen solo dentro de un rango de fechas activo.

**Programacion** — Una clase oficial cargada por Excel institucional. Es, junto con `ReservaSemestral`, una de las dos fuentes que pueden **generar** una `Llave` automáticamente cuando el docente pasa su credencial.

**ReservaSemestral** — Una franja recurrente semanal no oficial (un semillero, un club). Comparte estructura con `Programacion` (mismo `salon_id`/`dia`/`hora_inicio`/`hora_fin`) pero es una tabla separada — no comparten almacenamiento físico (decisión ya tomada en `estrategia-migracion/backend.md` para no romper el aislamiento de monolito modular). `grupo_id` agrupa las franjas creadas en una misma solicitud, para poder cancelarlas juntas.

**ReservaIndividual** — Una reserva de fecha puntual. Se aprueba sola al crearse (`estado` nunca pasa por un paso de aprobación manual) y puede generar una `Llave` igual que `Programacion`/`ReservaSemestral`.

**Llave** — La entidad más cargada de reglas de negocio. `docente_titular_id` es el dueño de la clase; `reclamado_por_id` es quien físicamente se llevó la llave (puede ser el mismo docente o un `Monitor` delegado — de ahí que sean dos campos, no uno). `origen` deja trazabilidad de cuál de las 3 fuentes de disponibilidad generó la entrega (o si fue manual). `ubicacion_entrega_id`/`ubicacion_devolucion_id` son una fotografía del momento — no se recalculan desde la ubicación actual del `Usuario`, porque esa puede cambiar después.

**Equipo** — Un ítem de inventario individual (un videobeam específico, no "los videobeams").

**Prestamo** — El préstamo de uno o varios `Equipo` a la vez, a diferencia de `Llave` (que es siempre un solo salón). Por eso `Prestamo` no tiene un único equipo asociado directamente — eso vive en `DetallePrestamo`.

**DetallePrestamo** — Un renglón por cada equipo dentro de un préstamo, con su propio estado de entrega/devolución. Es lo que permite la devolución **parcial**: un préstamo puede tener 2 de 3 equipos ya devueltos sin que el préstamo completo se cierre.

**Devolucion** — Un acto de devolución (puede haber varios por `Prestamo` si la devolución fue parcial en distintos momentos). `es_completa` marca si esa devolución en particular cerró todo el préstamo.

**Monitor** — La delegación de un docente titular hacia otra persona para reclamar/entregar su llave en su nombre. Acotada a `materia`+`aula`+`dia`+`horario` (una clase puntual), no a todas las clases del docente — para no sobre-otorgar acceso cuando el mismo docente dicta la misma materia en varias secciones.

**Novedad** — Un daño, pérdida u observación, asociada desde `Llave` o `DetallePrestamo` (la relación vive del lado de quien la generó, no al revés). `solucion` solo la puede definir Administrador; Auxiliar puede ver y cerrar (`estado`) pero no definirla.

**Notificacion** — Un recordatorio o aviso, automático o manual. `enviado_por_id` nulo significa que lo mandó el sistema solo; si tiene valor, fue un envío manual de Admin o Auxiliar.

**Configuracion** — Fila única (o pocas filas, si se quisiera por bloque) con los valores por defecto del sistema: umbral de mora, reintentos, plantilla de recordatorio y ubicación preseleccionada en formularios.

---

[Volver](../README.md)
